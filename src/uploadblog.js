...
constructor(props) {
	...
	validateUploadData(){
		...
		if(	this.state.header.length>headerLength||
	...
	handleUpload = (e) => {
		...
		contentdata.append('abstract', this.state.abstract.padEnd(abstractLength-this.state.abstractLength));
			...
			if(this.state.whattoupload==='blogcontent'){
				if(this.state.uploaderfolgreich){
					...
					return (
						...
						<br/>
							<form>
								<input type="file" id="abstractpicture" onChange={this.myPictureHandler} name="abstractpicture" accept="image/png, image/jpeg, image/jpg"></input>
							</form>
						<br/>
						<br/>
						<p>Content:</p>
						<br/>
						<SunEditor onChange={this.myEditorHandler} setOptions={{
							"mode": "classic",
							"rtl": false,
							"previewTemplate": "<div style='width:auto; max-width:80%; margin:auto;'>    <h1>Preview</h1>     {{contents}} </div>            ",
							"katex": "window.katex",
							"width": "100%",
							"minHeight": "400px",
			...
			if(this.state.whattoupload==='trinkspiel'){
				if(this.state.uploaderfolgreich){
					...
					return(
						...
						<button onClick={this.uploadMore}> Zurück </button>
						<h2>
							Uploade hier Trinkspielaufgaben.
						</h2> 
						<h4>
							Wenn ein Name eingesetzt werden soll schreibe $user.
							Lasse ein Leerzeichen nach $user. 
							Soll ein Name mehrmals vorkommen schreibe eine Zahl beginnend mit 1 direkt dahinter.
							z.B. "$user1 trinkt mit $user. $user1 springt danach im Kreis." 
							Achte darauf keine Zahl zu überspringen.
						</h4>
