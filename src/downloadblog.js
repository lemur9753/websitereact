...
class Downloadblog extends Component {
	...
	componentDidUpdate(prevProps, prevState) {
		if (
			prevState.cardnumbers !== this.state.cardnumbers ||
	...
	updateUrl() {
		this.props.history.push(
			`/home/cardnumber/${this.state.cardnumbers}/pagenumber/${this.state.pagenumber}`
		);
	}
		...
		axios({
			...
			.then((response) => {
				...
				for (let i = 0; i < this.state.index; i++) {
					if (
						this.state.authors[i].length > 0 &&
						this.state.headers[i].length > 0 &&
						this.state.abstracts[i].length > 0 &&
						this.state.pictures[i].length > 0
					) {
						cards.push(
							<Cards
								key={i}
				...
				<>
					<div style={{ display: 'flex', justifyContent: 'center' }}>
						<div style={{ display: 'flex', flexDirection: 'column' }}>
							<div style={{ display: 'flex' }}>{this.getContentSelector()}</div>
							<div style={{ display: 'flex' }}>{this.getPagination()}</div>
						</div>
					</div>
					<div style={{ margin: 20 }}>
						<Grid
