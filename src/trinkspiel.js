...
class Trinkspiel extends Component {
	...
	handleAufgabe() {
		...
		if (
			aufgabe.regel !== undefined &&
			aufgabe.regel.regelrunden > 0 &&
			this.checkIfRegelCanBePutInState(aufgabe)
		) {
	...
	handlePlayerClickClose() {
		let cleanedPlayerArray = this.state.players.filter(function (e) {
			return e;
		});
		this.setState({
			playerAddModalError: '',
	...
	getPlayerControl() {
		return (
			<div>
				<Button variant='contained' color='primary' onClick={() => this.handlePlayerClickOpen()}>
				...
				<Dialog open={this.state.playerAddModalOpen} onClose={() => this.handlePlayerClickClose()}>
					...
					/>
					<DialogTitle>Füge Spieler hinzu:</DialogTitle>
					<DialogContent>
	...
	getRegelAufgehoben() {
		return (
			<div>
				<Dialog
					open={this.state.regelAufhebeModalOpen}
					onClose={() => this.handleRegelAufgehobenClose()}
				>
					<DialogTitle>Folgende Regel wurde aufgehoben:</DialogTitle>
					<DialogContent>
						<div style={{ width: '100%', textAlign: 'center' }}>
							<h4>{this.state.regelAufhebeModaltext}</h4>
						</div>
						<Button
							variant='contained'
							color='primary'
		...
		return (
			<div>
				...
				<Dialog
					...
					fullWidth
				>
					<DialogTitle>Bestehende Regeln:</DialogTitle>
					<DialogContent>
				...
				<>
					<div className='topcontainer'>
						...
						<div className='controls'>
							...
							<div className='addplayerbutton'>{this.getPlayerControl()}</div>
							<div className='getregelbutton'>{this.getBestehendeRegeln()}</div>
						</div>
						<div className='drinkingcontainer'>
							<div className='aufgabe' style={{ whiteSpace: 'pre-line' }}>
								<h1>{this.state.jetzigeaufgabe}</h1>
							</div>
